...
func Create(volPath, snapDir, volName string) (*SnapInfo, error) {
	...
	accessPath := filepath.Join(snapDir, volName+"-cold-snap")
	info := &SnapInfo{
		VolName:    volName,
		SnapDir:    snapDir,
...
func ListOrphaned(snapDir string) ([]*SnapInfo, error) {
	...
	for _, e := range entries {
		name := e.Name()
		if !strings.HasSuffix(name, "-cold-snap") {
			continue
		}
		volName := strings.TrimSuffix(name, "-cold-snap")
