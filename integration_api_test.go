...
func TestAPI_EdgeCases(t *testing.T) {
	...
	resp := testutil.DoRequest(t, ts.URL, "POST", "/api/dummy-volume", map[string]string{"name": "badname"})
	resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 for plain name, got %d", resp.StatusCode)
	}

	resp = testutil.DoRequest(t, ts.URL, "PUT", "/api/volumes", nil)
	resp.Body.Close()
...
func TestAPI_SnapshotViewFallbackHash(t *testing.T) {
	...
	resp := testutil.DoRequest(t, ts.URL, "GET",
	...
	resp2 := testutil.DoRequest(t, ts.URL, "GET",
...
func TestAPI_SnapshotViewDiffFallbackHash(t *testing.T) {
	...
	respErr := testutil.DoRequest(t, ts.URL, "GET",
