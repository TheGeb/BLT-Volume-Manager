  ...
  <header class="topbar">
    ...
    <div class="topbar-actions">
      ...
      <button class="button-icon" title="Refresh" on:click={doRefresh} disabled={refreshing}>
        <span class="mask-icon" class:spin={refreshing} style="mask: url('/material/sync.svg') no-repeat center / contain; width:28px;height:28px;"></span>
      </button>
      <button class="button-icon" title="Migrate metadata between S3 and etcd backends" on:click={openMigrateModal}>
        <span class="mask-icon" style="mask: url('/material/swap_horiz.svg') no-repeat center / contain; width:28px;height:28px;"></span>
      </button>
      <div class="color-picker-wrapper">
        <button class="button-icon" title="Choose accent color" on:click|stopPropagation={() => showColorPicker = !showColorPicker}>
          <span class="mask-icon" style="mask: url('/material/palette.svg') no-repeat center / contain;"></span>
        </button>
        {#if showColorPicker}
      ...
      <button class="button-icon" title="Toggle light/dark mode" on:click={toggleTheme}>
        <span class="mask-icon" style="mask: url('/material/routine.svg') no-repeat center / contain;"></span>
      </button>
      <button class="button-icon" title="About" on:click={() => { showInfoModal = true; loadVersionInfo(); }}>
        <span class="mask-icon" style="mask: url('/material/info.svg') no-repeat center / contain;"></span>
      </button>
    </div>
  </header>
