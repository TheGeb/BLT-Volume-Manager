...
- Ensure `restic` **v0.17.0 or newer** is installed in the runtime image (the shipped Docker image pins restic 0.19.1). BLT-Volume-Manager relies on restic's documented exit code `10` for "repository does not exist", which restic only reports since v0.17.0. Set `RESTIC_REPOSITORY` and `RESTIC_PASSWORD` (and AWS credentials) for S3.
